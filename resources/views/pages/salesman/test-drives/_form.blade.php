...
<div class="row g-3">
    @if(!isset($testDrive))
        ...
        <div class="col-md-5"><label class="form-label" for="slot_date_filter">1. Select Date</label><input id="slot_date_filter" type="date" min="{{ today()->format('Y-m-d') }}" class="form-control" data-slot-date><div class="form-text">Choose a future date for the Test Drive.</div></div>
    ...
    @endif
    <div class="col-12"><label class="form-label">3. Available Test Drive Slot</label><div class="slot-grid" data-slot-grid>
        @foreach($slots as $slot)
            ...
            @if($belongsToVehicle)
                <label class="slot-option {{ $bookedByOther ? 'is-booked' : '' }}" data-slot-option data-vehicle="{{ $slot->vehicle_id }}" data-date="{{ $slot->slot_date->format('Y-m-d') }}">
                    <input type="radio" name="slot_id" value="{{ $slot->id }}" {{ (string) old('slot_id', $testDrive->slot_id ?? '') === (string) $slot->id ? 'checked' : '' }} {{ $bookedByOther ? 'disabled' : '' }}>
            ...
            @endif
        @endforeach
    </div>@error('slot_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror<div class="form-text" data-slot-empty>Select a date and Demo Vehicle to reveal its predefined Slots.</div></div>
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    const filter=()=>{let visible=0;options.forEach(option=>{const show=Boolean(vehicle.value&&date.value&&option.dataset.vehicle===vehicle.value&&option.dataset.date===date.value);option.hidden=!show;if(show)visible++;});empty.textContent=!date.value?'Select a future date.':!vehicle.value?'Select a Demo / Test Drive Vehicle.':visible?'Choose one available Test Drive Slot.':'No slots available for this date.';};
    vehicle.addEventListener('change',filter);date.addEventListener('change',filter);filter();
});
</script>
@endif
